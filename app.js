    ...
    if (!isLoggedIn) {
        ...
        modalContent = `
            ...
            <div class="modal-buttons">
                <button class="btn btn-discord" onclick="loginForGame('${game.id}')">
...
function loginForGame(gameId) {
    // Store game ID to show modal after login
    localStorage.setItem('pendingGameId', gameId);
    window.location.href = `${AUTH_BASE}/discord`;
}
...
async function init() {
    ...
    // Check for ?game=xxx parameter (from Discord link) or stored pending game from login
    ...
    if (gameParam) {
        ...
        // Wait for render, then scroll to game and show availability modal
        setTimeout(() => {
            handleGameLinkParam(gameParam);
        }, 300);
    } else if (pendingGameId && isAuthenticated) {
        // User just logged in after clicking Discord link - show the modal
        localStorage.removeItem('pendingGameId');

        setTimeout(() => {
            handleGameLinkParam(pendingGameId);
        }, 300);
    }
